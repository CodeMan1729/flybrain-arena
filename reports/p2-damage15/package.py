...
root=Path.cwd();b=root/'reports/p2-damage15';original=json.loads((b/'original-hashes.json').read_text());manifest={};diff=[]
...
records['scope']={'changed':'weapon_damage 10 to 15; seven hits defeat 100 HP; cooldown, speed, range unchanged','manual_playtest':'user requested 15 damage; restart pending','resource_warnings':'existing ObjectDB/resource cleanup warnings persist in physics suites'}
...
cmd=['git','apply','--unsafe-paths','--directory=reports/p2-damage15/patch-rehearsal',str(b/'changes.patch')]
...
assert json.loads((b/'verification.json').read_text())['modified-combat_round']['exit_code']==0
