...
func fire() -> void:
	...
	var hit_drone: bool = not hit.is_empty() and hit.collider is CharacterBody3D and hit.collider.has_method("take_damage") and hit.collider != self
	if hit_drone: hit.collider.take_damage(weapon_damage)
	fired.emit(hit_drone)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if active and event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT and Input.mouse_mode == Input.MOUSE_MODE_CAPTURED:
		fire()
