...
root=Path.cwd();b=root/'reports/p2-playtest';logs=b/'smoke-session';logs.mkdir(exist_ok=True)
...
try:
 with (logs/'brain.log').open('w',encoding='utf-8') as out:
  ...
  if p.returncode==0:
   shutil.copyfile(root/'reports/game-smoke.json',b/'game-smoke.json');print('passed:',json.loads((b/'game-smoke.json').read_text())['passed'])
finally:
