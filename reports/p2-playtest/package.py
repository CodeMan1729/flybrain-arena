...
root=Path.cwd();b=root/'reports/p2-playtest';original=json.loads((b/'original-hashes.json').read_text());manifest={};diff=[]
...
records['scope']={'changed':'flight_speed_scale=0.8; first-person gun, muzzle flash and recoil; HP/cooldown/raycast unchanged','manual_playtest':'initial feedback implemented; user restart and retest pending','resource_warnings':'existing ObjectDB/resource cleanup warnings persist in physics suites'}
...
cmd=['git','apply','--unsafe-paths','--directory=reports/p2-playtest/patch-rehearsal',str(b/'changes.patch')]
...
assert json.loads((b/'verification.json').read_text())['modified-isolated-smoke']['exit_code']==0
