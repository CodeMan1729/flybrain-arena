...
func _process(delta: float) -> void:
	...
	if player.active:
		...
		director.latest_telemetry["threat"] = preload("res://threat_sensor.gd").sample(fly,player)
...
func write_report(passed: bool, failure: String) -> void:
	...
	var result := {"passed":passed,"failure":failure,"checks":smoke_checks,"renderer":RenderingServer.get_video_adapter_name(),"display":DisplayServer.get_name(),"viewport":str(get_viewport().get_visible_rect().size),"frames":sorted.size(),"elapsed_seconds":elapsed,"mean_fps":1000.0*sorted.size()/sum if sum>0 else null,"godot_version":Engine.get_version_info(),"last_neural":director.neural,"mode":director.mode,"headless":DisplayServer.get_name()=="headless"}
	...
	if not brain_view_frame_times.is_empty():
		...
		result["brain_view_mean_fps"]=1000.0*brain_view_frame_times.size()/total if total>0 else null
