...
func _physics_process(delta: float) -> void:
	...
	var lateral := clampf((float(output[0])-float(output[1]))*15,-1,1) if fresh else 0.0
	var climb := clampf((float(output[2])+float(output[3]))*5,-0.45,0.45) if fresh else 0.0
	var speed := minf(absf(float(output[0]))*14,1.9) if fresh else 0.0
	if visible_player:
		...
		gaze=gaze.rotated(Vector3.UP,delta*(0.4+lateral))
		sight={}
	...
	if not fresh: desired=Vector3.ZERO
