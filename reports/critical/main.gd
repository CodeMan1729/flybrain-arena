...
func _ready() -> void:
	...
	AudioServer.set_bus_volume_db(0, linear_to_db(volume * 0.6))
...
func build_ui() -> void:
	...
	add_slider("Ses · üst sınır sınırlı",0,0.7,volume,func(v): volume=v; AudioServer.set_bus_volume_db(0,linear_to_db(maxf(v*0.6,0.0001))))
...
func start_game() -> void:
	...
	player.previous_yaw = player.rotation.y
	player.active = true
	fly.reset_body()
	fly.active = true
...
func can_interact(target: Vector3) -> bool:
	...
	return ray.length() < 2.5 and ray.normalized().dot(-player.camera.global_basis.z) > 0.65
	...
	if accepted and action_name != "wait":
		...
		match action_name:
			...
			"steps":
				...
				footsteps.volume_db = -12 + linear_to_db(maxf(intensity,0.01))
				footsteps.play()
			"silhouette":
...
func _process(delta: float) -> void:
	...
	memory_text.text += "\nTepki puanı korku ölçümü değildir.\nHer turun iyileşme getirmesi garanti değildir."
