...
func build_world() -> void:
	...
	label3("DRONE ARENA\nTEMASTAN KAÇIN",Vector3(-3,1.85,-5.78),27,Color(0.73,0.60,0.35))
...
func build_ui() -> void:
	...
	hud.add_child(badge)
	var controls := ui_label("W A S D  Hareket     F  El feneri     Sol tık  Ateş     B  Beyin     V  Beyni incele     TAB  Ölçümler     ESC  Duraklat",18)
	...
	column.add_child(ui_label("Sol alanda hareket · Sağ alanda bakış\nFener ve duraklatma ekranda. Ateş: masaüstü fare." if touch_mode else "WASD / Fare · Sol tık ateş · F el feneri",17))
	...
	if touch_mode:
		...
		subtitle.text = "Drone’u vur. Temastan kaçın.\n\nHareketlerin ve değerlendirmen anonim ortak öğrenmeye katılır. Kamera ve mikrofon kullanılmaz." if OS.has_feature("web") else "Drone’u vur. Temastan kaçın."
...
func end_round(outcome: String) -> void:
	...
	clear_effects()
	menu.visible = true
	menu_title.text = "KAZANDIN · Drone etkisiz." if round_outcome == "won" else "KAYBETTİN · Drone temas etti."
...
func _process(delta: float) -> void:
	...
	ui_clock=0
	if menu.visible:
		menu_title.text = ("KAZANDIN · Drone etkisiz." if round_outcome == "won" else "KAYBETTİN · Drone temas etti.") if completed else ("DURAKLATILDI · Arena bekler." if playing else "Drone'u vur. Temastan kaçın.")
	objective.text = "DRONE HP %d / %d" % [fly.hp,fly.max_hp]
	prompt.text = "" if not player.active or touch_mode else "SOL TIK · ATEŞ / TEMAS ÖLDÜRÜR"
	...
	if memory_text.is_visible_in_tree() and not director.memory.is_empty():
		...
		for item in saved.get("recent",[]).slice(-3):
			...
			memory_text.text += "%s · %d sn · %d olay · tepki %.2f\n" % [{"won":"Zafer","lost":"Yenilgi","quit":"Çıkıldı","connection_lost":"Kesinti","restarted":"Yeniden"}.get(item.get("outcome",""),"Tur"),int(item.get("seconds",0)),int(item.get("events",0)),mean_score]
...
func automated_run() -> void:
	...
	# Arrange an approaching contact, then let the real physics step cross the threshold.
	# This checks contact settlement, not autonomous pursuit from the default spawn.
	fly.position = player.position+Vector3(0,1.25,-0.65)
	fly.velocity = Vector3(0,0,4)
	fly.gaze = Vector3.BACK
	fly.set_physics_process(true)
	start = Time.get_ticks_msec()
	while not completed and Time.get_ticks_msec()-start<4000: await get_tree().physics_frame
	if not check(completed and round_outcome == "lost" and player.hp == 0 and menu.visible,"Physical drone contact returns to defeat menu"): return
	start = Time.get_ticks_msec()
	while director.ending and Time.get_ticks_msec()-start<4000: await get_tree().process_frame
