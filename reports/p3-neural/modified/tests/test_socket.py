        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                with (Path(tmp)/'godot-feedback.log').open('w') as game_output:
                    game=await asyncio.create_subprocess_exec(str(godot_path()),
                    ...
                    try:
                        self.assertEqual(await asyncio.wait_for(game.wait(),35),0,(Path(tmp)/'godot-feedback.log').read_text(encoding='utf-8'))
                    finally:
                ...
                lines=[json.loads(s) for f in Path(tmp).glob('*.jsonl') for s in f.read_text(encoding='utf-8').splitlines()]
