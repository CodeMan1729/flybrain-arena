...
func run_checks() -> void:
	...
	for screen in [Vector2i(320,568),Vector2i(390,844),Vector2i(844,390),Vector2i(568,320)]:
		...
		for action in ["lights","steps","silhouette"]:
			...
			if not readable:
				print("Layout diagnostic: ",occupied," text=",game.feedback_text.text," visible lines=",game.feedback_text.get_visible_line_count(),"/",game.feedback_text.get_line_count())
