...
from .escape import EscapePathway
...
class Connectome:
    ...
    def __init__(self, directory=ROOT / 'data'):
        ...
        self.escape = EscapePathway(directory, ids)
        self.info['escape'] = self.escape.info
    ...
    def step(self, values, threat=None):
        ...
        self.escape.inject(drive, threat)
        ...
        return {'output': out.tolist(), 'escape': self.escape.read(self.state), 'mean_abs': float(np.abs(self.state).mean()),
