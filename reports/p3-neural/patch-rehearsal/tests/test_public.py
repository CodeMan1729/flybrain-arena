        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                async with first, connect(uri, origin=origin, proxy=None) as other:
                    ...
                    await send(first, 'applied', id=1, accepted=True, telemetry=inputs[0])
                    await send(first, 'feedback', id=1, rating=2)
                    self.assertEqual(json.loads(await asyncio.wait_for(first.recv(), 2))['type'], 'error')
                    ...
                    async with connect(uri, origin=origin, proxy=None) as paused:
                        ...
                        await asyncio.sleep(2)
                        await send(paused, 'decision', id=39, telemetry=inputs[0])
                        self.assertNotEqual((await receive(paused, 'decision'))['action'], 'wait')
                        await asyncio.sleep(1.6)  # Past the 1.5-second application deadline.
                        await send(paused, 'applied', id=39, accepted=True, telemetry=inputs[0])
                        await send(paused, 'feedback', id=39, rating=2)
                        self.assertEqual(json.loads(await asyncio.wait_for(paused.recv(), 2))['type'], 'error')
                        self.assertEqual(json.loads((directory / 'learning-v3.json').read_text()), saved)
                        await asyncio.sleep(.4)
                        await send(paused, 'decision', id=40, telemetry=inputs[0])
                        next_event = await receive(paused, 'decision')
                        self.assertNotEqual(next_event['action'], 'wait')
                        self.assertEqual(next_event['budget_used'], 0)
                    ...
                    try:
                        ...
                        # A stale ACK for the previous round must not cancel the new pending event.
                        await send(other, 'applied', id=1, accepted=True, telemetry=inputs[1])
                        ...
                        opened = json.loads(await asyncio.wait_for(other.recv(), 2))
                        self.assertEqual((opened['type'], opened['id']), ('feedback_open', 51))
                    finally:
