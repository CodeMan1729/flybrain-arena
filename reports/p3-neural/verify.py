...
commands=[('baseline-escape',[python,'-c',"import brain.connectome as c; exec(compile(open('reports/p3-neural/original/brain/connectome.py',encoding='utf-8').read(),c.__file__,'exec'),c.__dict__); import unittest; unittest.main(module=None,argv=['baseline','tests.test_escape','-v'])"]),('python-all',[python,'-m','unittest','discover','-s','tests','-v'])]
for name in ['escape','combat_round','weapon_view','gameplay','pursuit','fly','hud','drone_visual','mobile','report','settings','brain_view']:
