...
func _ready() -> void:
	...
	add_button("DURAKLAT",game.pause_game,Vector2(88,48))
	add_button("ETKİLEŞ",game.interact,Vector2(88,56))
	add_button("FENER",func(): game.player.torch.visible = not game.player.torch.visible,Vector2(88,48))
	add_button("BEYİN",game.toggle_brain_details,Vector2(68,48))
	for rating in 3:
		ratings.append(add_button(["Etkilemedi","Gerildim","Korktum"][rating],func(): game.director.rate_event(rating),Vector2(92,48)))
...
func layout() -> void:
	...
	buttons[1].position = Vector2(size.x-64,size.y-110)
	buttons[2].position = Vector2(size.x-64,size.y-48)
	buttons[3].position = Vector2(size.x-60,94)
...
func _draw() -> void:
	...
	draw_string(ThemeDB.fallback_font,Vector2(36,size.y-29),"HAREKET",HORIZONTAL_ALIGNMENT_LEFT,-1,12,Color(0.7,0.82,0.76))
	draw_string(ThemeDB.fallback_font,Vector2(size.x-124,size.y-150),"SÜRÜKLE · BAKIŞ",HORIZONTAL_ALIGNMENT_LEFT,-1,12,Color(0.7,0.82,0.76))
